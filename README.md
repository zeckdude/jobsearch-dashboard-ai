...
`/sources` also supports supported niche job boards. JobFront-powered boards such as `https://jobs.frontdoordefense.com/` can be added with the job board form; the connector reads public job cards from the board API and does not log in or bypass paid/member gates.
