...
- Search completion triggers agency-first automation when it saves new eligible 90+ matches and no recruiting agency run is active. The handoff appends progress to the search run and starts the agency with `triggeredBy: "search_auto"`. Bulk preparation is limited to approved/generated-material jobs so `needs_review` exceptions cannot skip agency approval.
